...
Znear = min(Znears{cam_L + 1}, Znears{cam_R + 1});
Zfar = max(Zfars{cam_L + 1}, Zfars{cam_R + 1});
C_L_O = double(imread(C_L_O_path));
C_R_O = double(imread(C_R_O_path));
C_V_O = imread(C_V_O_path);
D_L_O = getDepthMap(imread(D_L_O_path),Znears{cam_L + 1}, Zfars{cam_L + 1});
D_R_O = getDepthMap(imread(D_R_O_path),Znears{cam_R + 1}, Zfars{cam_R + 1});
K_L = Ks{cam_L + 1};
K_R = Ks{cam_R + 1};
K_V = Ks{cam_V + 1};
Rt_L = Rts{cam_L + 1};
