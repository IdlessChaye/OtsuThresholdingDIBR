function [C, D] = getColorSwitchingDepthMedianFilter(C_O, D_O, w_select_radius, w_calc_radius)
    [H,W] = size(D_O);
    pixel_percent = 0.5;
...
%             C(i,j,:) = median_row(2:4);
            D(i,j,1) = median(d_chosen);
            C(i,j,1) = median(c_r_chosen);
            C(i,j,2) = median(c_g_chosen);
            C(i,j,3) = median(c_b_chosen);
        end
    end
end