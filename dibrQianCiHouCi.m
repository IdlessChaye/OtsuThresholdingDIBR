function [C_V] = dibrQianCiHouCi(layer_number, C_L_O, C_R_O, Z_L_O, Z_R_O, K_L_O, K_R_O, K_V_O, Rt_L_O, Rt_R_O, Rt_V_O, z_sign, is_dibr_do_refinement)
%     DIBR
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	layer_thresh_L = getAvgThresh(Z_L_O, layer_number - 1);
    layer_thresh_R = getAvgThresh(Z_R_O, layer_number - 1);
...
%             C_V_mi = C_Vs{i};
%             Z_V_mi = Z_Vs{i};
%             C_V_inpaints{i} = cat(3,medfilt2(C_V_mi(:,:,1),[2 * w_radius + 1, 2 * w_radius + 1]),medfilt2(C_V_mi(:,:,2),[2 * w_radius + 1, 2 * w_radius + 1]),medfilt2(C_V_mi(:,:,3),[2 * w_radius + 1, 2 * w_radius + 1]));
%             Z_V_inpaints{i} = medfilt2(Z_V_mi(:,:,1),[2 * w_radius + 1, 2 * w_radius + 1]);
        else
            C_V_inpaints{i} = C_Vs{i};
            Z_V_inpaints{i} = Z_Vs{i};     
        end
